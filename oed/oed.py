...
class OED(object):
    ...
    def __init__(self, model_fun, n_param, n_design, n_obs, 
        ...
        self.f = f
        # assert f_noise_scale >= 0, \
        #     "Invalid function noise scale: {0}".format(f_noise_scale)
        self.f_noise = f_noise
        assert n_dv > 0 and isinstance(n_dv, int), \
            "Invalid design variable space dimension: {0}".format(n_dv)
        self.n_dv = n_dv
        dv_bounds = np.array(dv_bounds)
        try:
            dv_bounds = dv_bounds.reshape((n_dv, 2))
        except BaseException:
            raise IndexError(
                "Invalid design variable bounds shape, correct shape is (n_dv,2)")
        try:
            dv_bounds_range = dv_bounds[:, 1] - dv_bounds[:, 0]
        except BaseException:
            raise TypeError(
                "Invalid type of design variable bounds' elements, correct type is int or float")
        assert np.min(dv_bounds_range) >= 0, \
            "Upper bounds of design variables is lower than lower bounds"
        self.dv_bounds = dv_bounds
        assert n_para > 0 and isinstance(n_para, int), \
            "Invalid parameter space dimension: {0}".format(n_para)
        self.n_para = n_para
        para_sample = np.array(para_sample)
        try:
            para_sample = para_sample.reshape((-1, n_para))
        except BaseException:
            raise IndexError(
                "Invalid parameter sample shape, correct shape is (n_sample, n_para)")
        n_sample = para_sample.shape[0]
        self.para_sample = para_sample
        # para_prior_ex = "Example: para_prior={'type':'uniform', 'loc':[0,1], 'scale':[1,1]}"
        # assert isinstance(para_prior, dict), \
        #     "Invalid type of parameter prior, correct type is dict\n" + para_prior_ex
        # para_prior_type_set = {"uniform", "normal"}
        # para_prior_type = para_prior["type"]
        # assert para_prior_type in para_prior_type_set, \
        #     "Parameter prior type not in set {0}".format(para_prior_type_set)
        # assert len(para_prior["loc"]) == n_para and len(para_prior["scale"]) == n_para, \
        #     "Length of parameter prior location or scale list not equals n_para"
        # self.para_prior = para_prior
        if n_draw is not None:
            assert n_draw >= 2 and isinstance(n_draw, int), \
                "Invalid draw grid number: {0}".format(n_draw)
            assert n_dv <= 3, \
                "Cannot draw utility function if design space dimension is bigger than 2"
        opt_method_set = {"L-BFGS-B", "TNC", "SLSQP", "Bayesian-Optimization"}
        if opt_method is None:
            opt_method = "L-BFGS-B"
        else:
            assert opt_method in opt_method_set, \
                "Invalid optimization method {0}".format(opt_method)
        ...
        self.cons = cons
        self.dv_hist = np.array([])
        self.para_hist = np.array([])
        self.y_hist = np.array([])
        if dv_noise_scale_prior is not None:
            self.dv_noise_scale_prior = dv_noise_scale_prior
            self.dv_noise_scale_hist = np.array([])
        self.noise = None
        self.optimizer = None
        self.add_penalty = add_penalty
        self.sequential = sequential
        # if sequential:
        self._r = np.ones(n_sample)
        self._c = np.ones(n_sample)
        self.estimate_evid = estimate_evid
        self.random_state = random_state
